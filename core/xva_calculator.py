"""Unilateral CVA / FVA on a toy option-style positive exposure.

`paths` is 2-D after the backend collapses the basket:
    shape (n_steps, n_paths) = portfolio value through time.
"""
...
def calculate_xva_metrics_gpu(
    ...
    # Reduced-form WWR: hazard rises in current exposure.
    # Not a structural coupling to the counterparty's asset.
    ...
    if short_rates is not None:
        # pathwise DF from integrating the short rate
        integral_r = torch.cumsum(short_rates * dt, dim=0)
        df = torch.exp(-integral_r)
    else:
